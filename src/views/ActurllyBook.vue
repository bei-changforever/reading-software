    ...
    <div class="act">
        ...
        <div class="padding-box">
            <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
                <div class="common-bookele" v-for="(item, index) in actList" :key="index" @click="bookHandler(item)">
                    <div class="img">
...
<script>
import { getAct } from '@/apis/homeApi/homeApi'
export default {
    name: 'Act',
    data() {
        return {
    ...
    methods: {
        async getActListItem() {
            let { data: { data } } = await this.axios(getAct(this.options))
            this.actList = data.books
        ...
        onLoad() {
            ...
            this.timer = setTimeout(async () => {
                this.options.page++
                let { data: { data } } = await this.axios(getAct(this.options))
                this.actList = [...this.actList, ...data.books]
                this.loading = false
